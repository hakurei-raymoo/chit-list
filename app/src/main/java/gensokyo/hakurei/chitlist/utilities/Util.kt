...
object Converter {
    ...
    fun removeDecimal(string: String): Int {
        ...
        formatted = when (Config.decimalOffset) {
            3 -> "%.3f".format(float)
            2 -> "%.2f".format(float)
            1 -> "%.1f".format(float)
            else -> float.toString()
